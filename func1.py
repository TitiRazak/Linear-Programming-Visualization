    ...
    if not check_entries():
        ...
        if prob.status == 1:
            ...
            if c[1] != 0:
                ...
                # Plot parallel lines from the objective function
                for i in range(-5, 6):
                    y_vals_parallel = [(z - c[0] * x - i) / c[1] for z, x in zip(z_vals, x_vals)]
                    ax.plot(x_vals, y_vals_parallel, 'g--', alpha=0.3)
                ...
                # Plot parallel lines from the objective function
                for i in range(-5, 6):
                    ax.axvline(pulp.value(prob.objective) / c[0] + i, color='g', linestyle='--', alpha=0.3)

            # Annotate the objective function with the optimal solution
            ax.annotate(f'{c[0]}x + {c[1]}y = {pulp.value(prob.objective):.2f}', (max_x * 0.7, max_y * 0.7),
                        textcoords="offset points", xytext=(0, 10), ha='center')
